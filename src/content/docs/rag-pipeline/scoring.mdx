...
<Aside type="note">
  The formula is <code>e^(-age_days / 30)</code>. At 30 days the score is <code>e^(-1) ≈ 0.37</code>
  , not exactly 0.5 — this is steeper than a true half-life but works well for news content where a
  month is old.
</Aside>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        <code>0.8×0.82 + 0.2×1.00 = 0.86</code>
      </td>
    </tr>
    <tr>
      <td>Article B (old)</td>
      ...
      <td>
        <code>0.8×0.91 + 0.2×0.14 = 0.76</code>
      </td>
    </tr>
  </tbody>
</table>
...
<Aside type="tip">
  Over-fetching is cheap — S3 Vectors returns results in single-digit milliseconds. The reranking
  step adds negligible latency but significantly improves answer quality for time-sensitive queries.
</Aside>
