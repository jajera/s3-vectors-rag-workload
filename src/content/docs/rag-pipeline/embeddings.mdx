...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        <Tooltip term="cosine" label="Cosine similarity" />
      </td>
    </tr>
    <tr>
      <td>Access</td>
      <td>
        Amazon <Tooltip term="bedrock" label="Bedrock" /> runtime API
      </td>
    </tr>
  </tbody>
</table>
...
<Aside type="note">
  The 8000-character limit is a practical safety margin below Titan V2's 8192-token maximum.
  Characters and tokens aren't 1:1, but for ASCII English text after normalization the ratio is
  close enough that 8000 characters stays safely within the token budget.
</Aside>
...
<Aside type="tip">
  For production systems with longer documents (PDFs, technical manuals), chunking with overlap is
  usually necessary to stay within token limits and improve retrieval precision.
</Aside>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Store vector in <Tooltip term="s3-vectors" label="S3 Vectors" /> index
      </td>
    </tr>
    <tr>
      <td>Query</td>
...
<Aside type="caution">
  If you change the embedding model, you must re-ingest the entire corpus. Vectors from different
  models are not comparable — similarity scores become meaningless.
</Aside>
