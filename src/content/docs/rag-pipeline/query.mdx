...
<Aside type="tip">
  The actual retrieval fetches `desired_top_k × RETRIEVAL_MULTIPLIER` vectors (capped at 20). This
  over-fetch ensures the reranking stage has enough candidates to choose from after filtering and
  deduplication.
</Aside>
...
<Aside type="note">
  Documents with no publish date receive a default recency score of 0.25 — roughly equivalent to a
  42-day-old article. This prevents undated documents from being unfairly penalized or boosted.
</Aside>
...
<Aside type="caution">
  If no candidates survive filtering (empty result set), the pipeline returns a structured fallback
  message explaining that no matching announcements were found — it never sends an empty context to
  the LLM.
</Aside>
...
<Aside type="tip">
  The <code>ask()</code> function is invoked by the Query Lambda handler. The Lambda receives a
  JWT-authenticated request via API Gateway, extracts the question, calls <code>ask()</code>, and
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <code>RETRIEVAL_MULTIPLIER</code>
      </td>
      <td>3</td>
    ...
    <tr>
      <td>
        <code>RECENCY_HALF_LIFE_DAYS</code>
      </td>
      <td>30</td>
