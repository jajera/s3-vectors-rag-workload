...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        The <Tooltip term="embedding-model" label="Titan Embeddings V2" /> or LLM has not been
        enabled in the Bedrock console for your account and region.
      </td>
    </tr>
    <tr>
      <td>
        IAM policy missing <code>bedrock:InvokeModel</code>
      </td>
      <td>
        The Lambda execution role does not include the <code>bedrock:InvokeModel</code> action for
        the model ARN.
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td>
        <Tooltip term="inference-profile" label="Inference profile" /> not available
      </td>
      <td>
        Cross-region inference profiles require model access enabled in ALL regions the profile
        routes to.
      </td>
    </tr>
  </tbody>
...
<Tooltip term="bedrock" label="Bedrock" /> enforces per-model, per-account rate limits (tokens per
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Ingesting many documents at once exceeds the default requests-per-minute quota for{" "}
        <Tooltip term="titan-v2" label="Titan V2" />.
      </td>
    </tr>
    <tr>
      <td>Account-level quota too low</td>
      <td>
        Default Bedrock quotas are conservative for new accounts (e.g., 100 RPM for embedding
        models).
      </td>
    </tr>
    <tr>
...
<Aside type="note">
  If you see ThrottlingException in logs but the function completes successfully, the retries
  handled it. Only investigate if the function returns an error or times out.
</Aside>
...
<Aside type="tip">
  Find the exact quota code in the Service Quotas console under Amazon Bedrock → Titan Embeddings V2
  model invocations.
</Aside>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        An empty string or None was passed as <code>inputText</code> to the embedding model.
      </td>
    </tr>
    <tr>
      <td>Text exceeds token limit</td>
      <td>
        Titan Embeddings V2 supports up to 8,192 tokens per input. Very long documents without
    ...
    <tr>
      ...
      <td>
        Missing required fields, wrong data types, or invalid JSON in the <code>InvokeModel</code>{" "}
        body.
      </td>
    </tr>
  </tbody>
...
<Tooltip term="titan-v2" label="Titan Embeddings V2" /> accepts up to 8,192 tokens (roughly 30,000
...
<Aside type="caution">
  If an RSS feed item has no content (only a title), the ingest function may pass an empty string to
  the embedding model. The code should skip items with empty body text.
</Aside>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        Running <code>terraform apply</code> immediately after enabling model access may hit this
        window.
      </td>
    </tr>
  </tbody>
...
<Aside type="tip">
  If you are deploying via Terraform for the first time, enable model access in the Bedrock console
  at least 10 minutes before running <code>terraform apply</code>. This avoids the Lambda
  bootstrapping against an unavailable model.
</Aside>
...
<Aside type="note">
  If ServiceUnavailableException persists for more than 30 minutes and no AWS Health event is
  reported, open an AWS Support case.
</Aside>
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Typo in the model identifier (e.g., <code>amazon.titan-embed-text-v2</code> instead of{" "}
    ...
    <tr>
      ...
      <td>
        The ARN references a profile that was never created, was deleted, or is in a different
        account.
      </td>
    </tr>
    <tr>
      ...
      <td>
        Some models are only available in specific regions. Using a model ID in an unsupported
        region returns this error.
      </td>
    </tr>
  </tbody>
</table>
