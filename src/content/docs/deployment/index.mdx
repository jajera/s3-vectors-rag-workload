...
<Tooltip term="terraform" label="Terraform" /> resolves resource dependencies automatically and
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>
        <strong>3. Scheduling + Hosting</strong>
      </td>
      <td>~15s</td>
    ...
    <tr>
      <td>
        <strong>4. Bootstrap ingest</strong>
      </td>
      <td>~60–90s</td>
...
<Aside type="note">
  Phases 1–3 run with Terraform parallelism. Phases 4 and 5 are sequential — the bootstrap ingest
  needs the vector index to exist, and the web UI needs Cognito and API Gateway outputs.
</Aside>
...
<Aside type="caution">
  The web UI deploy runs on your local machine (not in AWS). It requires Node.js and npm to be
...
<Aside type="tip">
  If the bootstrap ingest takes longer than 2 minutes, check your network connectivity and Bedrock
  API throttling. The ingest function makes ~120 embedding API calls.
</Aside>
