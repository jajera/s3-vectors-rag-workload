...
<Aside type="note">
  The generated `config.js` is gitignored. It only exists locally after `terraform apply` and inside
  the Amplify deployment artifact.
</Aside>
...
<Aside type="tip">
  Manual deployment means Amplify serves static files uploaded via the CLI — it does not pull from a
  Git repo or run a build pipeline.
</Aside>
...
<Aside type="caution">
  The <code>-replace</code> flag destroys and recreates the resource, which re-runs the deploy
  script. This causes a brief period where the old deployment is torn down before the new one is
  live.
</Aside>
