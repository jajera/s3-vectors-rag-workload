...
<Aside type="note">
  Docker is <strong>not</strong> required. Lambda deployment packages are built from source using
  Terraform's <code>archive_file</code> data source.
</Aside>
...
<Aside type="caution">
  Not all regions support S3 Vectors or all Bedrock models. Verify availability before changing the
  region.
</Aside>
